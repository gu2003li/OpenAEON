...
function resolveGatewayPort(cfg: OpenAEONPluginApi["config"]): number {
  const envPort =
    parsePositiveInteger(process.env.OPENAEON_GATEWAY_PORT?.trim()) ??
    parsePositiveInteger(process.env.AEONPROPHET_GATEWAY_PORT?.trim());
  if (envPort) {
    return envPort;
  }
...
function resolveAuth(cfg: OpenAEONPluginApi["config"]): ResolveAuthResult {
  const mode = cfg.gateway?.auth?.mode;
  const token =
    pickFirstDefined([
      process.env.OPENAEON_GATEWAY_TOKEN,
      process.env.AEONPROPHET_GATEWAY_TOKEN,
      cfg.gateway?.auth?.token,
    ]) ?? undefined;
  const password =
    pickFirstDefined([
      process.env.OPENAEON_GATEWAY_PASSWORD,
      process.env.AEONPROPHET_GATEWAY_PASSWORD,
...
async function resolveGatewayUrl(api: OpenAEONPluginApi): Promise<ResolveUrlResult> {
  ...
  api.registerCommand({
    ...
    handler: async (ctx) => {
      ...
      if (action === "qr") {
        ...
        if (channel === "telegram" && target) {
          try {
            ...
            if (send) {
              await send(
                target,
                ["Scan this QR code with the OpenAEON iOS app:", "", "```", qrAscii, "```"].join(
                  "\n",
                ),
                {
        ...
        return {
          text: [
            "Scan this QR code with the OpenAEON iOS app:",
            "",
            "```",
            qrAscii,
            "```",
