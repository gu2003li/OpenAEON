...
import type { OpenAEONPluginApi, OpenAEONPluginService } from "openaeon/plugin-sdk";
...
function normalizeDenyList(cfg: OpenAEONPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: OpenAEONPluginApi["config"]): string[] {
...
export default function register(api: OpenAEONPluginApi) {
  ...
  const timerService: OpenAEONPluginService = {
